...
import { getTotalNewsBySearch } from '../pagination/pagination-v2';
...
function onSearch(e) {
  ...
  if (!encoded) {
    Notiflix.Notify.warning(
      'Empty search! Please enter some criteria for search.'
    );
    return;
  }
...
export default async function getResponseNewsBySearch(curPage) {
  try {
    const response = await searchApiService.getNewsBySearch(curPage - 1);
    ...
    Notiflix.Notify.warning(
      'No response news by search from server. Please, try again later.'
    );
    console.log(error);
  }
}
    ...
    markup += `
		...
		<div class="news-card" id="${docs[i]._id || curPage * 10 + i}">
            ...
            docs[i].multimedia.length == 0
              ? DEFAULT_PHOTO
              : DEFAULT_BASE_URL + docs[i].multimedia[0].url
          }"
					alt="${
            docs[i].keywords.length == 0
              ? DEFAULT_CAPTION
              : docs[i].keywords[0].value
          }"
					height="395" />
