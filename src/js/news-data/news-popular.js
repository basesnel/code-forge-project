...
export default async function getResponsePopular(curPage) {
  try {
    ...
    renderMainNewsList(response.results, weather, curPage);
  } catch (error) {
    Notiflix.Notify.warning(
      'No response popular news from server. Please, try again later.'
    );
    console.log(error);
  }
}
...
function renderMainNewsList(
  ...
  let markup = '';
  let start = curPage * (newsPerPage - 1) - (newsPerPage - 1);
  let end = curPage * (newsPerPage - 1);
  let weatherMarkup = '';
  ...
  if (!weatherBlock) {
    weatherMarkup = `<li class="list-news__item">
  <div class="weather">
